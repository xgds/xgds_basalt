...
urlpatterns = [url(r'^login/$', auth.views.login, {}, 'user-login'),
               ...
               url(r'^reset-password-confirm/(?P<uidb36>[^/]+)/(?P<token>.+)$', auth.views.password_reset_confirm, {}, 'password_reset_confirm'),
                #url(r'^reset-password-confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$', auth.views.password_reset_confirm, name='password_reset_confirm'),
