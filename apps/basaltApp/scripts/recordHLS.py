...
class HLSRecorder:
    ...
    def __init__(self, teradekUrl, m3u8DirPath, recorderId):
        self.teradekUrl = teradekUrl
        self.m3u8DirPath = m3u8DirPath
        self.recorderId = recorderId

        self.m3u8Filename = os.path.basename(teradekUrl)
    ...
    def initRecording(self):
        s = self.httpSession
        m3u8String = s.get(self.teradekUrl).text
        ...
        for seg in self.m3u8Obj.segments:
            videoData = s.get("%s/%s" % (os.path.dirname(self.teradekUrl),
                                         seg.uri))
    ...
    def recordNextBlock(self, sleepAfterRecord=True):
        ...
        try:
            ...
            print "%s %s %s" % ("recordHLS:",
                                "*** Warning: Exception polling Teradek.",
                                "Trying again... ***")
            return  # skip and give teradek a break
...
def main():
    import optparse
    parser = optparse.OptionParser('usage: %prog --teradekUrl <m3u8 URL> ' + 
                                   '--outputDir <dirname> ' +
                                   '--recorderId <recorderID>')
    parser.add_option('--teradekUrl', dest="teradekUrl",
                      help='URL to m3u8 file on teradek')
    ...
    if (not opts.teradekUrl) or (not opts.outputDir) or (not opts.recorderId):
        parser.error("All options are required")

    print "Start HLS recording:"
    print "  Recorder ID:", opts.recorderId
    print "  Teradek URL:", opts.teradekUrl
    print "  Output path:", opts.outputDir

    hlsRecorder = HLSRecorder(opts.teradekUrl, opts.outputDir, opts.recorderId)
